...
def analyze(ast: dict) -> dict:
    ...
    def visit(node: Optional[dict]):
        ...
        if meta == 'func':
            ...
            if current_scope.lookup_local(fn_name):
                err(f"Redeclaración: función '{fn_name}' ya fue declarada en este scope",
                    fn_name, node_ln)

            params = [c for c in children if not c.get('children') and c.get('meta') in ('', 'id', None)]
        ...
        if meta == 'call':
            ...
            if fn_label and fn_label not in _BUILTINS:
        ...
        # ── Bucles ────────────────────────────────────────────────────────────
        if meta in ('while', 'for'):
            prev_in_loop = in_loop
            in_loop = True
            push_scope(f'{label}_loop')
            for child in children:
                visit(child)
            pop_scope()
        ...
        if meta == 'op' and len(children) == 2:
            ...
            # ⑧ Conversión implícita inválida: bool en operación aritmética
            if label in _ARITH_OPS and label not in ('+',):
                if lt == 'bool' or rt == 'bool':
                    err(
                        f"Conversión implícita inválida: operador '{label}' "
                        f"recibe tipo 'bool', que no es numérico",
                        label, node_ln,
                    )
