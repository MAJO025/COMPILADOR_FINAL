...
class TACGenerator:
    ...
    def visit(self, node: dict | None) -> str:
        ...
        if label in ('def', 'function', 'FUNCTION'):
            fname = _label(children[0]) if children else 'func'
            params = []
            body_nodes = []
            for ch in children[1:]:
                if _label(ch) in ('params', 'param', 'parameters') or ch.get('meta') == 'params':
                    params = [_label(p) for p in ch.get('children', [])]
                else:
                    body_nodes.append(ch)

            self._emit(f'FUNC_BEGIN {fname}', comment=f'params: {", ".join(params) or "ninguno"}', ln=ln)
            for p in params:
                self._emit(f'PARAM {p}', ln=ln)
            for b in body_nodes:
                self.visit(b)
        ...
        # ── Llamada a función ─────────────────────────────────────────────
        if label == 'call' or (label not in _ASSIGN and len(children) >= 1
                               and children[0].get('meta') == 'args'):
            if label == 'call':
                fname = _label(children[0]) if children else '?'
                args  = children[1].get('children', []) if len(children) > 1 else []
            else:
                fname = label
                args  = children[0].get('children', []) if children else []

            arg_temps = [self.visit(a) for a in args]
            for a in arg_temps:
                self._emit(f'ARG {a}', ln=ln)
            t = self._new_temp()
            self._emit(f'{t} = CALL {fname}, {len(arg_temps)}', ln=ln)
            return t
...
class StackGenerator:
    ...
    def visit(self, node: dict | None):
        ...
        if label in ('def', 'function', 'FUNCTION'):
            ...
            self._emit(f'FUNC_BEGIN {fname}', comment=f'params: {", ".join(params) or "ninguno"}', ln=ln)
            for p in params:
                self._emit(f'POP {p}', comment='recibir parámetro', ln=ln)
            for b in body_nodes:
                self.visit(b)
            self._emit(f'FUNC_END {fname}', ln=ln)
            return
        ...
        if label == 'call' or (label not in _ASSIGN and len(children) >= 1
            ...
            for a in args:
                self.visit(a)
            self._emit(f'CALL {fname}, {len(args)}', ln=ln)
            return
