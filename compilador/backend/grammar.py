...
PRECEDENCE: dict[str, Prec] = {
    ...
    # Nivel 5 — Bit a bit OR  (menor prioridad entre los bit a bit)
    '|':   Prec(5, 'L'),

    # Nivel 6 — Bit a bit XOR
    '^':   Prec(6, 'L'),

    # Nivel 7 — Bit a bit AND
    '&':   Prec(7, 'L'),

    # Nivel 8 — Desplazamientos de bits
    '<<':  Prec(8, 'L'),  '>>':  Prec(8, 'L'),

    # Nivel 9 — Aditivos
    '+':   Prec(9, 'L'),  '-':   Prec(9, 'L'),

    # Nivel 10 — Multiplicativos
    '*':   Prec(10, 'L'),  '/':   Prec(10, 'L'),
    '%':   Prec(10, 'L'),  '//':  Prec(10, 'L'),

    # Nivel 11 — Potencia  (asociativa a la DERECHA: 2**3**4 = 2**(3**4))
    #            Debe ser MAYOR que todos los binarios (incluido bit a bit).
    '**':  Prec(11, 'R'),

    # Nivel 12 — Unarios (mayor prioridad, asociativos a la derecha)
    'not':     Prec(12, 'R'),
    '!':       Prec(12, 'R'),
    'unary-':  Prec(12, 'R'),
    '~':       Prec(12, 'R'),
