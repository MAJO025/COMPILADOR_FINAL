...
function _drawASTAtZoom(ast, zoom) {
  const cv     = document.getElementById('astCanvas');
  const scroll = document.getElementById('astScroll');
  ...
  const treeW      = (b.mxX - b.mnX) * zoom;
  const containerW = scroll ? scroll.clientWidth : 600;
  const cssW = Math.max(treeW + PAD * 2 * zoom, containerW);
  const cssH = Math.max((b.mxY + PAD * 2) * zoom, 300);

  const centerOffsetPx = (cssW - treeW) / 2;
  const shiftX = centerOffsetPx / zoom - b.mnX;
