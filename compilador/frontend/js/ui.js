...
function setPhase(p) {
  curPhase = p;
  ['lexico','sintactico','semantico','intermedio','optimizacion','objeto'].forEach(ph => {
    const view = document.getElementById('view-' + ph);
    if (view) view.style.display = 'none';
    const btn = document.getElementById('btn-' + ph);
    if (btn) { btn.className = 'pbtn'; if (ph === p) btn.classList.add('active-' + p); }
  });
